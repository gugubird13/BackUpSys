...
void Packer::DfsFile(FileBase &bak_file, std::filesystem::path cur_path)
{
    ...
    if(file.IsHardLink())
    {
        if(inode_table.count(fileheader.metadata.st_ino)) // 检测是否存在
        {
            //指向的inode我已经打包过了，这里我记录一下文件的路径
        ...
        {
            // 指向的inode并没有被打包, 已经被解引用,直接当作普通文件处理即可
    ...
    case FILE_TYPE_NORMAL:
        bak_file.write((const char *)&fileheader, sizeof(fileheader));
        file.OpenFile(std::ios::binary | std::ios::in);
...
bool Packer::Pack()
{
    ...
    DfsFile(bak_file, std::filesystem::relative(root_path, root_path.parent_path())); // 返回的是 ../ 但是要符合path格式
    bak_file.close();
    return true;
}
...
{
    if (bak_path.extension() != FILE_PACKER_SUFFIX)
        return false;
